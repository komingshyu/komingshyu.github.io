document.addEventListener('DOMContentLoaded', function() {
    ...
    function saveDataToLocalStorage() {
        ...
        localStorage.setItem('subjects', JSON.stringify(subjects));  // 儲存科目代號及科目名稱
    ...
    function sortTeachers() {
        // 從 localStorage 獲取教師資料
     let savedTeachers = localStorage.getItem('teachers');
    if (savedTeachers) {
        teachers = JSON.parse(savedTeachers);
        // 依據教師ID排序
        teachers.sort((a, b) => a.id - b.id);
        console.log('Sorted teachers:', teachers); // 監控 teachers 變數
        saveDataToLocalStorage(); // 儲存排序後的教師資料
        updateTeacherTable(); // 更新教師表格
        showSortCompleteModal(); // 顯示排序完成的模態視窗
    }
   }

    // 班級排序
    function sortClasses() {
         // 從 localStorage 獲取班級資料
     let savedClasses = localStorage.getItem('classes');
    if (savedClasses) {
        classes = JSON.parse(savedClasses);
        // 依據導師姓名排序
        classes.sort((a, b) => a.mentor.localeCompare(b.mentor));
        console.log('Sorted classes:', classes); // 監控 classes 變數
        saveDataToLocalStorage(); // 儲存排序後的班級資料
   ...
   // 顯示排序完成的模態視窗
function showSortCompleteModal() {
    const modal = document.getElementById('sortCompleteModal');
        ...
        cls.subjects.forEach(subject => {
            if (subject.teacher) {
                ...
                if (teacher) {
                   ...
                   console.log('Current Subject Name:', subject.name);
                   const subjectCode = subjects.find(sub => sub.name === subject.name)?.code || '';
                   console.log('Found Subject Code:', subjectCode);
                    const row = [cls.mentor,  subjectCode, teacher.id,subject.name, teacher.name]; // 新增科目名稱欄位
...
function loadDemoData() {
    ...
    // 檢查教師代號或姓名是否已存在
    const existingTeacherById = teachers.some(teacher => teacher.id === id);
    const existingTeacherByName = teachers.some(teacher => teacher.name === name);

    if (existingTeacherById) {
        alert(`教師代號 ${id} 已經存在，請使用不同的代號。`);
        return; // 返回，停止新增
    }

    if (existingTeacherByName) {
        alert(`教師姓名 ${name} 已經存在，請使用不同的姓名。`);
        return; // 返回，停止新增
    }
    ...
    function addSubject() {
        ...
        // 檢查科目名稱和節數是否已存在於 headers 中
    const subjectIdentifier = `${subjectName}${subjectHours}`; // 組合科目名稱和節數
    const existingSubject = headers.some(header => header === subjectIdentifier);

    if (existingSubject) {
        alert(`科目名稱 ${subjectName} + 節數 ${subjectHours} 已經存在，請使用不同的科目名稱或節數。`);
        return; // 返回，停止新增
    }
...
function showEditSubjectModal() {
    ...
    // 清空之前的內容
    const editSubjectList = document.getElementById('editSubjectList');
    editSubjectList.innerHTML = '';

    // 檢查 localStorage 中是否有 subjects
    const storedSubjects = JSON.parse(localStorage.getItem('subjects'));
    if (!storedSubjects || storedSubjects.length === 0) {
        // 如果 subjects 為空，從 headers 中初始化
        subjects = headers.slice(2).map(header => ({
            code: '000',  // 預設科目代號
            name: header
        }));
    } else {
        subjects = storedSubjects; // 使用從 localStorage 讀取的 subjects
        // 僅檢查 headers 是否有 subjects 所缺少的資料
        headers.slice(2).forEach(header => {
            if (!subjects.some(sub => sub.name === header)) {
                subjects.push({
                    code: '000',  // 新增的科目代號預設為 '000'
                    name: header
                });
            }
        });
    ...
    // 顯示所有科目名稱及代號，並添加可編輯的輸入框
    subjects.forEach((sub, index) => {
        editSubjectList.innerHTML += `
            <div>
                <label>科目代號:</label>
                <input type="text" value="${sub.code}" id="subjectCode${index}" />
                <label>科目名稱:</label>
                <input type="text" value="${sub.name}" id="subjectName${index}" readonly />
            </div>
        `;
    });
}
...
function editSubject() {
    subjects.forEach((sub, index) => {
        const codeInput = document.getElementById(`subjectCode${index}`);
        
        // 更新科目代號
        if (codeInput) {
            sub.code = codeInput.value; // 更新科目代號
        }
    });

    saveDataToLocalStorage(); // 儲存更新後的 subjects
    //updateSubjectsTable(); // 更新顯示的科目表格
    closeEditSubjectModal(); // 關閉編輯模態視窗
}
...
function closeEditSubjectModal() {
 ...
 // 顯示科目代號及名稱模態視窗
    function showSubjectsTableModal() {
        const modal = document.getElementById('subjectsTableModal');
        modal.style.display = "block";
        updateSubjectsTable();
    }

    // 關閉科目代號及名稱模態視窗
    function closeSubjectsTableModal() {
        const modal = document.getElementById('subjectsTableModal');
        modal.style.display = "none";
    }
    ...
    const mentorName = document.getElementById('newMentorName').value;

    // 檢查班級名稱或導師姓名是否已存在
    const existingClassByName = classes.some(cls => cls.class_name === className);
    const existingClassByMentor = classes.some(cls => cls.mentor === mentorName);

    if (existingClassByMentor) {
        alert(`班級名稱 ${mentorName} 已經存在，請使用不同的班級名稱。`);
        return; // 返回，停止新增
    }

    if (existingClassByName) {
        alert(`導師姓名 ${className} 已經存在，請使用不同的導師姓名。`);
        return; // 返回，停止新增
    }
    ...
    window.onload = function() {
       ...
       if (savedSubjects) {
        ...
        const closeBtn1 = document.querySelector('#sortCompleteModal .close-btn');
        closeBtn1.addEventListener('click', closeSortCompleteModal);
        const closeBtn2 = document.querySelector('#editSubjectModal .close-btn');
        closeBtn2.addEventListener('click', closeEditSubjectModal);
    ...
    document.getElementById('editSubjectButton').addEventListener('click', showEditSubjectModal);
    document.getElementById('sortTeachersButton').addEventListener('click', sortTeachers);
    document.getElementById('sortClassesButton').addEventListener('click', sortClasses);
   
    // 增加取消按鈕的功能
   document.getElementById('editSubjectConfirmButton').addEventListener('click', editSubject);
   document.getElementById('editSubjectCancelButton').addEventListener('click', closeEditSubjectModal);
