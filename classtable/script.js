document.addEventListener('DOMContentLoaded', function() {
    ...
    window.showDeleteTeacherModal = function(teacherId) {
        ...
        const modalContent = modal.querySelector('.modal-content');
        modalContent.innerHTML = `<h2>確認刪除教師</h2><p>您確定要刪除教師編號為 ${teacherId} 的教師嗎？</p><button onclick="deleteTeacher('${teacherId}')">確認刪除</button><button onclick="closeDeleteTeacherModal()">取消</button>`;
        modal.style.display = "block";
    }
    ...
    window.showDeleteSubjectModal = function(subjectIndex) {
        ...
        const modalContent = modal.querySelector('.modal-content');
        modalContent.innerHTML = `<h2>確認刪除科目</h2><p>您確定要刪除科目 ${headers[subjectIndex]} 嗎？</p><button onclick="deleteSubject(${subjectIndex})">確認刪除</button><button onclick="closeDeleteSubjectModal()">取消</button>`;
        modal.style.display = "block";
        modal.style.zIndex = 1000; // 確保模態視窗在最上層
    }
